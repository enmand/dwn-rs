...
pub trait Ordorable {
    fn to_order<'a, 's>(self) -> &'s [(&'a str, bool)];
}
    ...
    fn to_order<'a, 's>(self) -> &'s [(&'a str, bool)] {
        match self {
            MessageSort::DateCreated(direction) => {
                [("dateCreated", direction.to_bool())].as_slice()
            }

            MessageSort::DatePublished(direction) => {
                [("datePublished", direction.to_bool())].as_slice()
            }
            MessageSort::Timestamp(direction) => {
                [("messageTimestamp", direction.to_bool())].as_slice()
            }
        }
    }
}
...
impl Ordorable for MessageCidSort {
    fn to_order<'a, 's>(self) -> &'s [(&'a str, bool)] {
        [("messageCid", self.direction.to_bool())].as_slice()
    }
}
